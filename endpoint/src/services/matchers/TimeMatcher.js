...
export class TimeMatcher {
    ...
    getRecommendedTimes(normalizedScores) {
        ...
        const eligible = sortedTimes.filter(item => item.score >= absoluteThreshold && item.score >= maxScore - relativeThreshold);
        const middaySlots = eligible
            .filter(item => item.hour >= 10 && item.hour <= 14)
            .sort((a, b) => a.hour - b.hour);
        const eveningSlots = eligible.filter(item => item.hour >= 15);
        const otherSlots = eligible.filter(item => !middaySlots.includes(item) && !eveningSlots.includes(item));

        const recommended = [];
        if (middaySlots.length > 0) recommended.push(middaySlots[0]);
        if (middaySlots.length > 1) recommended.push(middaySlots[1]);
        if (recommended.length < this.config.maxRecommendations && eveningSlots.length > 0) {
            recommended.push(eveningSlots[0]);
        }
        const remaining = [...middaySlots.slice(2), ...eveningSlots.slice(1), ...otherSlots];
        for (const slot of remaining) {
            if (recommended.length >= this.config.maxRecommendations) break;
            if (!recommended.some(item => item.hour === slot.hour)) {
                recommended.push(slot);
            }
        }
        if (recommended.length === 0 && sortedTimes.length > 0) {
            recommended.push(sortedTimes[0]);
        }
        recommended.sort((a, b) => a.hour - b.hour);
        return recommended.slice(0, this.config.maxRecommendations);
    }
