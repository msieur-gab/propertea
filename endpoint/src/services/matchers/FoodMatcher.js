...
export class FoodMatcher {
    ...
    addFoodWithTrace(trace, scoreMap, food, score, reasonStep, reasonDetail) {
        ...
        if (!this._scoreBreakdown) {
            this._scoreBreakdown = new Map();
        }
        if (score > 0) {
            const breakdown = this._scoreBreakdown.get(normalizedFood) || {
                hints: 0,
                specific: 0,
                category: 0,
                mouthfeel: 0,
            ...
            if (reasonStep === "Flavor Pairing Hint") breakdown.hints++;
            else if (reasonStep === "Flavor-Specific Pairing") breakdown.specific++;
            else if (reasonStep === "Category-Based Pairing") breakdown.category++;
            else if (reasonStep.includes("Mouthfeel")) breakdown.mouthfeel++;
            else if (reasonStep.includes("Intensity")) breakdown.intensity++;
            else if (reasonStep.includes("Roast")) breakdown.roast++;
            this._scoreBreakdown.set(normalizedFood, breakdown);
        }
    ...
    getRecommendedPairings(normalizedScores) {
        ...
        const topFoods = sortedFoods
            ...
            .map(([food, score]) => {
                const breakdown = this._scoreBreakdown?.get(food) || {};
                const weight = (breakdown.hints || 0) * 3 +
                  (breakdown.specific || 0) * 2 +
                  (breakdown.category || 0) * 2 +
                  (breakdown.mouthfeel || 0) +
                  (breakdown.intensity || 0) +
                  (breakdown.roast || 0);
                return { name: food, score, weight };
            });
        
        // Fallback if filtering leaves nothing
        if (topFoods.length === 0 && sortedFoods.length > 0) {
            const fallback = sortedFoods[0];
            return [{ name: fallback[0], score: fallback[1], weight: 0 }];
        }
